...
export default function Home() {
  return (
    <main className="relative w-full h-screen overflow-y-scroll [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
      ...
      {/* --- 4. MAIN CONTENT --- */}
      {/* pt-48 on mobile and lg:pt-32 on desktop moves the content further down from the top */}
      <div className="relative min-h-screen flex items-start lg:items-center justify-center pt-48 pb-24 lg:pt-32 z-10 w-full">
        <div className="container mx-auto px-4 flex flex-col lg:flex-row items-center justify-between gap-y-12 lg:gap-x-12">
          ...
          <div className="w-full lg:w-[55%] flex flex-col justify-center text-center lg:text-left z-20">
            {/* Name - Now with Glitch Animation */}
            <motion.h1
              variants={fadeIn("down", 0.2)}
              initial="hidden"
              animate="show"
              exit="hidden"
              data-text="Aryan Sarda"
              className="animate-glitch text-6xl md:text-7xl lg:text-8xl font-black italic uppercase text-white mb-2 tracking-tighter leading-none drop-shadow-[6px_6px_0px_#ff2a2a] [text-shadow:3px_3px_0px_#00ffff]"
            >
              Aryan <br className="lg:hidden" /> Sarda
            </motion.h1>

            {/* Degree Sticker Highlight */}
